...
def read_trace_data(segy_file,
    ...
    if(sort_order not in ('inline', 'crossline')):
        raise RuntimeError(f'{sort_order} not supported, sort order should be on of inline or crossline.')
    ...
    def header_to_int(trace_as_bytes, scalco):
        ...
        hdr = {key:struct.unpack(val[2], hdr[val[0]-1:val[0]-1+val[1]])[0] for key,val in byte_locations.items()}
    ...
    for key, val in line_coords.items():
        ...
        output_file = os.path.join(filename, f'{sort_order}s', f'{key}', 'index.bin')
...
def compressed_zarr(segy_file, sort_order='inline'):
    ...
    bounds = root.create_dataset("bounds", data=[int(inlines.min()),
                                                 int(crosslines.min()),
                                                 int(inlines.max()),
                                                 int(crosslines.max())], 
    ...
    scalers = line_root.zeros("scalers", shape=(max_line - min_line + 1, 2), dtype=int)
    ...
    for line in tqdm.tqdm(glob(folder)):
        ...
        traces = np.fromfile(os.path.join(filename, f'{sort_order}s', f'{line_number}', 'traces.bin'), 
                    dtype='<f4')
        indx = np.fromfile(os.path.join(filename, f'{sort_order}s', f'{line_number}', 'index.bin'),
                    dtype=bool)
